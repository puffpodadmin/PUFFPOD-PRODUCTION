...
module.exports=async function handler(req,res){
  ...
  try{
    ...
    const userRef=db.collection('users').doc(decoded.uid),withdrawalRef=db.collection('referralWithdrawals').doc();
    let amount=0,userData={};
    await db.runTransaction(async tx=>{
      const snap=await tx.get(userRef); if(!snap.exists) throw new Error('Cadastro não encontrado.');
      userData=snap.data()||{}; if(!userData.referralEnabled) throw new Error('Seu programa de indicação não está ativo.');
      amount=Math.floor((Number(userData.referralAvailable||0)+Number.EPSILON)*100)/100;
      if(amount<5) throw new Error('O saque mínimo é R$ 5,00.');
      tx.set(withdrawalRef,{uid:decoded.uid,userName:userData.name||decoded.name||'',userEmail:userData.email||decoded.email||'',amount,pixType,pixKey,status:'pending',createdAt:admin.firestore.FieldValue.serverTimestamp(),paymentProvider:null,mode:'manual_pending_gateway'});
    ...
    return res.status(200).json({ok:true,withdrawalId:withdrawalRef.id,amount,status:'pending',message:'Solicitação de saque registrada. O pagamento será processado após a ativação do novo gateway.'});
