...
const { sendWithdrawalPaidEmail, sendWithdrawalRejectedEmail } = require('../lib/mailer');
...
module.exports=async function handler(req,res){
  ...
  try{
    ...
    // Ações sobre solicitações de saque de indicação (fluxo manual)
    if(action==='markPaid'||action==='reject'){
      ...
      if(action==='reject' && !reason) return res.status(400).json({error:'Informe uma justificativa para o estorno.'});
      ...
      await db.runTransaction(async tx=>{
        ...
        const u=us.data()||{};
        emailInfo={
          to:w.userEmail||u.email,
          name:w.userName||u.name,
          amount,pixType:w.pixType,pixKey:w.pixKey,reason
        };
        if(action==='markPaid'){
          ...
          tx.update(wr,{status:'rejected',rejectedAt:admin.firestore.FieldValue.serverTimestamp(),rejectedBy:decoded.email||ADMIN_EMAIL,rejectionReason:reason});
      ...
      try{
        if(action==='markPaid') await sendWithdrawalPaidEmail(emailInfo);
        else await sendWithdrawalRejectedEmail(emailInfo);
        emailSent=true;
      }catch(mailErr){
        console.error('withdrawal admin email',mailErr);
        emailError=mailErr.message||'Falha ao enviar e-mail.';
      }
      await wr.set({[action==='markPaid'?'paidEmailSent':'rejectedEmailSent']:emailSent},{merge:true});
