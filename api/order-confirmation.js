...
async function getOrder(orderId, token){
  ...
  return {
    ...
    total: Number(firestoreValue(f.total) || 0)
  };
}
...
async function verifyInfinitePayPaid({orderId, transactionNsu, slug}){
  const handle = String(process.env.INFINITEPAY_HANDLE || '').replace(/^\$/,'').trim();
  if(!handle) throw new Error('INFINITEPAY_HANDLE não configurada na Vercel.');
  if(!transactionNsu || !slug) throw new Error('Dados da transação InfinitePay ausentes.');
  const payload={ handle, order_nsu:String(orderId), transaction_nsu:String(transactionNsu), slug:String(slug) };
  ...
  for(const endpoint of endpoints){
    ...
    try{
      const upstream=await fetch(endpoint,{method:'POST',headers:{'Content-Type':'application/json','Accept':'application/json','User-Agent':'Puffpod/1.0'},body:JSON.stringify(payload),signal:controller.signal});
      const raw=await upstream.text(); let data; try{data=JSON.parse(raw);}catch{data={};}
      if(upstream.ok){
        if(!data.success || !data.paid) throw new Error('Pagamento ainda não está confirmado pela InfinitePay.');
        return data;
      }
      if(upstream.status>=400&&upstream.status<500&&upstream.status!==404&&upstream.status!==405) throw new Error(data.error||data.message||'Falha ao validar pagamento na InfinitePay.');
      last=new Error(data.error||data.message||('HTTP '+upstream.status));
    }catch(err){last=err;}finally{clearTimeout(timeout);}
  }
  throw last || new Error('Não foi possível consultar a InfinitePay agora.');
}
...
async function processReferralCommission(orderId){
  const admin=getAdmin(); const db=admin.firestore(); const orderRef=db.collection('orders').doc(String(orderId));
  let result={credited:false};
  await db.runTransaction(async tx=>{
    const os=await tx.get(orderRef); if(!os.exists) return;
    const order=os.data()||{}; if(order.referralCommissionProcessed) return;
    const ref=order.referral||{}; const referrerUid=String(ref.referrerUid||'');
    if(!referrerUid){ tx.set(orderRef,{referralCommissionProcessed:true,referralCommissionAmount:0},{merge:true}); return; }
    const userRef=db.collection('users').doc(referrerUid); const us=await tx.get(userRef);
    ...
    const u=us.data()||{}; const percent=Number(u.referralCommissionPercent||0);
    if(![10,15,20].includes(percent)){ tx.set(orderRef,{referralCommissionStatus:'waiting_rate'},{merge:true}); return; }
    const base=Number(order.subtotal||0); const amount=Math.round((base*percent/100)*100)/100;
...
module.exports = async function handler(req, res){
  ...
  try{
    ...
    if(!orderId || !transaction_nsu || !slug) return res.status(400).json({error:'Pedido ou dados da transação ausentes.'});

    const order = await getOrder(String(orderId), token);
    const notificationsAlreadySent = order.emailSent && order.whatsappSent;

    const payment = await verifyInfinitePayPaid({orderId, transactionNsu:transaction_nsu, slug});
    const expectedCents = Math.round(Number(order.total || 0) * 100);
    if(expectedCents > 0 && Number(payment.amount) !== expectedCents) throw new Error('O valor confirmado pela InfinitePay não corresponde ao total do pedido.');
    let referralCommission=null;
    try{ referralCommission=await processReferralCommission(String(orderId)); }catch(e){ console.error('referral commission error',e); }
    if(notificationsAlreadySent) return res.status(200).json({ok:true, alreadySent:true, emailSent:true, whatsappSent:true, referralCommission});
    const firstName = String(order.name || 'cliente').trim().split(/\s+/)[0] || 'cliente';
    const amount = moneyFromCents(payment.amount);
