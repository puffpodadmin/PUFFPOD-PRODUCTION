...
module.exports=async function handler(req,res){
  ...
  try{
    ...
    if(order.paymentStatus!=='paid') return res.status(409).json({error:'Pagamento ainda não confirmado pela MGXPay.'});
    ...
    if(!emailSent){
      try{
        ...
        await transporter.sendMail({from:`Puffpod <${STORE_EMAIL}>`,to:order.email,replyTo:STORE_EMAIL,subject:`Pagamento confirmado — Pedido ${orderId}`,text:`Olá, ${firstName}!\n\nSeu pagamento foi confirmado com sucesso.\n\nPedido: ${orderId}\nValor pago: ${amount}\n\nSeu produto está a caminho.\n\nPuffpod`,html:`<div style="font-family:Arial,sans-serif;background:#061d2c;padding:28px;color:#eaf6fb"><div style="max-width:560px;margin:auto;background:#0b3550;border:1px solid #1d5573;border-radius:18px;padding:28px"><div style="font-size:12px;letter-spacing:2px;color:#48cae4;text-transform:uppercase">Puffpod</div><h1>Pagamento confirmado ✓</h1><p>Olá, <strong>${firstName}</strong>!</p><p>Recebemos seu pagamento via MGXPay.</p><div style="background:#06283d;border-radius:12px;padding:16px"><div>CÓDIGO DO PEDIDO</div><div style="font-family:monospace;font-size:19px;color:#67e8f9">${orderId}</div><div style="margin-top:12px">Valor pago: <strong>${amount}</strong></div></div></div></div>`});
