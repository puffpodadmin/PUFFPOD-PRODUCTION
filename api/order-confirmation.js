...
async function verifyInfinitePayPaid({orderId, transactionNsu, slug}){
  ...
  const upstream = await fetch('https://api.checkout.infinitepay.io/payment_check', {
    ...
    body: JSON.stringify({ handle, order_nsu:String(orderId), transaction_nsu:String(transactionNsu), slug:String(slug) })
  });
  const raw = await upstream.text();
  let data; try { data = JSON.parse(raw); } catch { data = {}; }
  if(!upstream.ok) throw new Error(data.error || data.message || 'Falha ao validar pagamento na InfinitePay.');
